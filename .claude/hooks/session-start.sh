...
elif [ -d "synapse/src" ]; then
    TODO_COUNT=$(grep -r "TODO" synapse/src/ 2>/dev/null | wc -l)
    FIXME_COUNT=$(grep -r "FIXME" synapse/src/ 2>/dev/null | wc -l)
    if [ "$TODO_COUNT" -gt 0 ] || [ "$FIXME_COUNT" -gt 0 ]; then
        echo ""
        echo "Code health: ${TODO_COUNT} TODOs, ${FIXME_COUNT} FIXMEs in synapse/src/"
    fi
fi
